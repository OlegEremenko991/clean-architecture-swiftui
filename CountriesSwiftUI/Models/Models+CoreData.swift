...
extension Locale {
    ...
    var shortIdentifier: String {
        .init(identifier.prefix(2))
    }
}

extension Country.Details {
    init?(managedObject: CountryDetailsMO) {
        guard let capital = managedObject.capital else {
            return nil
        }
...
extension Country.Details.Intermediate {
    ...
    func store(
        ...
        guard let details = CountryDetailsMO.insertNew(in: context) else {
            return nil
        }
...
extension Country {
    ...
    func store(in context: NSManagedObjectContext) -> CountryMO? {
        guard let country = CountryMO.insertNew(in: context) else {
            return nil
        }
        country.name = name
        ...
        let translations = translations.compactMap { (locale, name) -> NameTranslationMO? in
            guard let name = name,
    ...
    init?(managedObject: CountryMO) {
        guard let nameTranslations = managedObject.nameTranslations else {
            return nil
        }
